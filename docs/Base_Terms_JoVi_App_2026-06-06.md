# Terms of Use for the JoVi App

Legal version: 2026-06-06.base.v1
Approved for app version: 2.1.8+261601438

Version date: 06.06.2026

## 1. Provider

The provider of the JoVi App and contractual partner for base use is:

Johannes Vilsmeier  
Blütenstr. 11
71384 Weinstadt  
Email: developer.joviapp@gmail.com
## 2. Scope of These Base Terms

These Base Terms apply to the general local use of the JoVi App without enabled cloud or Pro features. They cover in particular solo use on the end device, local data handling, and generally available export, open, share, help, and scan functions.

Separate Cloud Terms additionally apply to cloud features such as sign-in. Separate Pro Terms additionally apply to paid Pro features and subscriptions such as team synchronization and server-supported collaboration. From a technical perspective, Pro features are always also cloud features.

## 3. Base Services of the App

The base use of the JoVi App supports local order, organization, and statistics workflows, especially in gastronomy, event, or club environments.

Depending on the locally available feature set, the app may in particular provide the following services:

- local management of menus, table orders, and payment data
- local statistics and evaluations based on entered data
- export, import, sharing, and opening of files
- QR-based or camera-supported scan functions
- access to help, community, and legal-text content

There is no claim to a specific feature set unless it has been expressly promised.

## 4. Permitted Use

You may use the app only in compliance with statutory law and these Terms of Use. In particular, the following is prohibited:

- use for unlawful purposes
- bypassing technical protection measures
- entering, processing, or distributing unlawful content
- using the app in a manner that impairs the technical stability of the app or integrated services

## 5. Responsibility for Content and Operational Use

You are responsible for the content you enter, modify, export, or share yourself. This applies in particular to:

- menu contents
- order and payment data
- notes and other free-text entries
- exported or shared files

If you use the app within a business, club, or other organization, you are responsible for ensuring that the use is legally permissible within your sphere of responsibility. This especially concerns data protection, labor, tax, bookkeeping, or retention obligations.

## 6. User Entries, Statistics, Exports, and Generated Content

The app processes, stores, visualizes, and exports only those data that are entered, imported, confirmed, changed, or otherwise processed within the app by users. The provider does not assume any warranty or guarantee that the data entered by users or processed within the app are correct, complete, truthful, current, or free of contradictions.

The responsibility for all entries, specifications, corrections, assignments, notes, menu contents, orders, payment data, cancellations, claims, free-text entries, team data, and any other content entered into or processed via the app lies exclusively with the respective user or with the business or organization using the app.

The provider is not obliged to verify user entries, business transactions, orders, payments, revenues, cancellations, complimentary items, subsequent entries, stock movements, personnel actions, or other operational processes for factual correctness, completeness, plausibility, or consistency with actual circumstances.

All statistics, key figures, evaluations, reports, summaries, overviews, logs, PDF, CSV, HTML, JSON, or other export files, as well as all other generated content displayed or created by the app, are based solely on the data stored in the app.

The provider assumes no warranty or guarantee for the factual, mathematical, commercial, operational, tax, accounting, legal, or other professional correctness, completeness, timeliness, reliability, usability, or suitability of such content for a specific purpose.

In particular, statistics, evaluations, reports, export files, and other generated content do not constitute a binding representation of actual sales, revenues, payment processes, stock movements, work performance, or other real business events. Their informative value depends entirely on whether all relevant events have been fully, correctly, promptly, and truthfully recorded in the app.

Transactions, payments, sales, cancellations, subsequent entries, complimentary items, cash adjustments, corrections, external cash collections, or other business events that take place outside the app or are not recorded in the app, not recorded correctly, not recorded completely, or not recorded in time, will not be reflected, or not be reflected correctly, in statistics, evaluations, reports, exports, and other generated content.

Exported or generated files serve solely as a technical work, organization, information, and documentation aid. They do not constitute an assurance of specific properties, proof of actual business events, or suitability for tax, accounting, legal, business, or official purposes.

The user is obliged to independently review all statistics, evaluations, reports, export files, and other generated content before using, sharing, archiving, billing with, using for accounting or tax purposes, or otherwise processing them further.

Unless expressly agreed otherwise in writing, the app is not a legally required, certified, or professionally audited cash register system, bookkeeping software, tax or financial documentation system, merchandise management system, or legally secure evidence or archiving system.

In particular, the provider does not owe any legal, tax, accounting, or regulatory suitability of the app or the content generated by it.

## 7. Availability and Technical Requirements

The provider strives to ensure the app is available as uninterruptedly as possible. However, uninterrupted, error-free, and complete availability is not owed.

Use may in particular be impaired by:

- maintenance and updates
- failures of networks, end devices, or platform services
- restrictions of platform or third-party services
- security-related blocks or technical protection measures

## 8. External Content and Additional Features

The app may provide external content such as community texts, online legal texts, or embedded videos. Such external offerings may be subject to additional terms and privacy information of the respective third-party providers.

Cloud and Pro features are not governed by these Base Terms. If you activate such features, the corresponding separate terms additionally apply.

## 9. Intellectual Property and Rights of Use

Unless otherwise indicated, all rights to the app, the software, the design, the texts, and other components remain with the provider or the respective rights holders.

By providing the app, you are granted a simple, non-transferable, revocable right to use it for its intended purpose within the framework of these Terms of Use.

## 10. Liability

The provider is liable without limitation:

- in cases of intent and gross negligence
- in the event of injury to life, body, or health
- under mandatory statutory liability provisions

In the event of slightly negligent breach of essential contractual obligations, liability is limited to the typically foreseeable damage. Otherwise, liability for slight negligence is excluded to the extent permitted by law.

To the extent permitted by law, the provider is not liable for damages, consequential damages, financial losses, business interruptions, wrong decisions, duplicate entries, incorrect dispositions, data loss, lost profits, tax disadvantages, booking errors, cash discrepancies, or other disadvantages resulting from data in the app being incorrect, incomplete, delayed, or not recorded at all, from business events having taken place outside the app, or from statistics, evaluations, reports, export files, or other generated content being incorrect, incomplete, outdated, or unsuitable for the intended purpose, unless there is intent, gross negligence, or another mandatory statutory basis of liability.

You are responsible for regularly backing up important data. The provider is only liable for data loss within the framework of the liability rules set out above.

## 11. Privacy

Information on the processing of personal data can be found in the separate Privacy Policy of the JoVi App.

## 12. Online Dispute Resolution and Consumer Dispute Resolution

The European Commission provides an online dispute resolution platform:

https://ec.europa.eu/consumers/odr/

The provider is neither obliged nor willing to participate in dispute resolution proceedings before a consumer arbitration board.

## 13. Changes to These Base Terms

The provider may amend these Terms of Use for the future if factual reasons require it, in particular in the event of:

- further development of the app
- changes in sales channels or store requirements
- changes in legal requirements
- changes in external services or technical framework conditions

Users should be informed of material changes in an appropriate manner.

## 14. Final Provisions

The law of the Federal Republic of Germany applies, excluding the UN Convention on Contracts for the International Sale of Goods, unless mandatory consumer protection provisions provide otherwise.

If the user is a merchant, a legal entity under public law, or a special fund under public law, the place of jurisdiction is the provider's registered office, to the extent permitted by law.

Should any provision of these Terms of Use be or become wholly or partially invalid, the validity of the remaining provisions shall remain unaffected.

































